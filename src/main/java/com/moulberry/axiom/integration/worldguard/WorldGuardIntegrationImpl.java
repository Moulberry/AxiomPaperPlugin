...
public class WorldGuardIntegrationImpl {
    ...
    static SectionPermissionChecker checkSection(Player player, World world, int cx, int cy, int cz) {
        ...
        Iterable<ProtectedRegion> regions = regionManager.getApplicableRegions(test, RegionQuery.QueryOption.COMPUTE_PARENTS).getRegions();
        ProtectedRegion globalRegion = regionManager.getRegion("__global__");
        if (globalRegion != null) {
            regions = Iterables.concat(regions, Collections.singletonList(globalRegion));
        }
